...
> 状态：源码已具备指令层 GUI（M1–M5）、场景库页、overlay-only 部署与 fixture scaffold 页；冻结 sidecar 内嵌 `fixture_packs/`。当前桌面包线是 unsigned `desktop-v0.6.0-beta.1`。正式签名、公证与实体设备验收仍待完成
...
| 本轮交付 | **指令层 M1–M5 + 场景库 + overlay-only GUI** | 部署页内置稿固定 overlay（#76）；夹具页只调用 CLI scaffold；sidecar 内嵌 `fixture_packs/`；`cli_runner` 超时覆盖 leader 退出后仍被占用的管道（#80）；目标 unsigned tag `desktop-v0.6.0-beta.1` |
...
**参数表（来自 `codex-instruct.py` argparse，v0.6.0）：**
...
以下为兼容基线 CLI 输出样例（最初采自 v0.2.0），0.6.0 GUI 解析器必须能处理。
...
| **Desktop overlay-only 与夹具页** ✅ | Deploy 内置稿固定 overlay、本地文件模式、Fixtures 列表/预览/写入/删除、sidecar 内嵌 fixture packs、#80 超时/进程树 | GUI 只调用既有 CLI；部署参数互斥；夹具预览绑定 pack/目录/force 并明确未修改 `~/.codex`；目标 tag 为 `desktop-v0.6.0-beta.1` |

## 10. 交接说明（给接手 Agent）

**起点：** canonical 仓库内的 `gui/` 目录；CLI 源码固定取仓库根目录 `codex-instruct.py`，GUI 与 CLI 可绑定到同一提交。

**当前交付（截至 2026-09-21，v0.6.0）：**
...
- CLI 源码：仓库根目录 `codex-instruct.py`（v0.6.0，单文件）
